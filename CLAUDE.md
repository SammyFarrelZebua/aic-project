...
1. Clears `root_cause_predictions` / `complaint_prediction`, fetches all reviews via a paginated `fetchAllReviews()` helper and reads `analytics_traceability_view` via a generic `selectAll<T>()` helper — PostgREST's default `max_rows` (1000) silently truncates a bare `.select('*')`, so both reads page through `.range()` in chunks of 1000 rather than fetching in one call. This was a real production bug (pipeline was silently only processing the first 1,000 reviews) fixed 2026-08-17; keep this pattern if you touch these fetches.
