...
* **Source Dataset**: Olist Brazilian E-Commerce Dataset, subset to ~15,000 orders. Review text is **generated** — a hand-written Indonesian clause corpus (`utils/review-corpus.ts`), not machine-translated (an earlier scripted PT->ID translator produced many duplicate/empty texts and was replaced; see "Data generation"). The UI and NLP labels are Indonesian.
...
* Playwright E2E tests live under `tests/`. Run with `npx playwright test --workers=1` (the config runs all 4 browser projects with unbounded parallelism by default, which reliably causes cross-project races); requires a running dev server (`npm run dev`) and a seeded database with an admin account (`npm run ingest` + `npx tsx scripts/seed-admin.ts`).
...
4. Scores candidates with a **hybrid composite score**: `0.6 * isolationForestScore + 0.4 * (deviationRatio * incidentComplaintShare)`, restricted to the entity type structurally responsible for that incident category (`PRODUCT_DEFECT` → factories only, `PACKAGING_DAMAGE` → warehouses only, `LATE_DELIVERY` → couriers only — fixed 2026-08-22; it previously scored all 15 factory+warehouse+courier candidates together for every incident type, which took Top-1 identification accuracy from 22.08% to 64.94% on `npm run test-baseline` once fixed). The isolation-forest component (`utils/isolation-forest-detector.ts`) builds a 5-dim entity feature vector and normalizes outlier scores to [0,1]. Inserts the top candidate per anomaly into `root_cause_predictions`.
...
* `app/(dashboard)/dashboard/page.tsx` — client component fetching `/api/analytics/dashboard` on mount, "Run Pipeline" button that POSTs `/api/pipeline/run` and polls `/api/pipeline/status`, KPI cards, a Recharts complaint-trend chart, a top-suspects ranking panel, and a TraceStrip pipeline visual. The Run POST goes through a `postWithRetry` helper (added 2026-08-25) that retries transient network failures against the single-threaded dev server and maps genuine timeouts to a clearer message via `lib/pipeline-messages.ts`'s `describeConnectionFailure` — see "Pipeline-run resilience" note below.
...
Also see `PIPELINE_ARCHITECTURE.md` (project root) for a full architecture writeup.
...
* `next.config.ts` carries two Windows-dev-only workarounds (committed as of 2026-08-25; they were previously uncommitted): `distDir: '.next-dev'` (sidesteps an EPERM lock on `.next/dev/logs/next-development.log` that crashes `next dev` on this machine) and `experimental: { mcpServer: false }` (disables Next 16's experimental dev-mode MCP server, which writes to that same log path). `tsconfig.json`'s `include` and `.gitignore` were updated in tandem to cover the new `.next-dev/` output dir. These have no effect on production builds/behavior.
* `PIPELINE_ARCHITECTURE.md` is a line-cited architecture map of the whole system and the canonical deep-dive reference.
* `.commandcode/` (agent-tooling config: a permissions allowlist + style preferences) is **ignored** (`gitignore`) and not tracked. A 2026-08-25 merge brought it in from `origin/main`, but on this machine's exFAT drive the directory got a filesystem-level lock that blocked git operations, so it was removed from the merge and gitignored. It does not affect the app.
...
* A few UI rough edges worth knowing about if you touch those pages: `app/(dashboard)/reviews` and `app/(dashboard)/settings` have no `loading.tsx`/`error.tsx` (every other dashboard section does); the three entity detail pages (`entities/{factories,warehouses,couriers}/[id]/page.tsx`) render a plain "not found" div instead of calling `notFound()`, and each has a non-functional "Lihat semua N ..." span with no href/onClick (they do use the shared `components/page-header.tsx` `PageHeader` component now — added by the 2026-08-25 "benerin frontend" merge — as do `alerts`, `cases`, `reviews`, `settings`, and the entity list pages).
* `components/page-header.tsx` — a shared `PageHeader` (title + optional description/icon/children) introduced by the 2026-08-25 merge of `origin/main` "benerin frontend". Used by `alerts`, `cases`, `reviews`, `settings`, and the three entity list + detail pages to standardize page headers.
* `components/sidebar.tsx`'s mobile hamburger (`aria-label="Buka menu"`) and mobile-close (`aria-label="Tutup menu"`) icon buttons were unlabeled until 2026-08-22 — fixed as a small accessibility improvement (found while debugging Playwright's `mobile-chromium` project, whose viewport puts the whole sidebar behind that closed drawer by default).
...
* **Pipeline-run resilience (2026-08-25):** the dashboard's "Run Pipeline" POST goes through `postWithRetry` (`app/(dashboard)/dashboard/page.tsx`), which retries transient network failures against the single-threaded dev server (the fetch can be reset/timed-out client-side while the server is busy inside the CPU-bound classification pass). It never retries on HTTP errors or genuine timeouts. `lib/pipeline-messages.ts` gained `describeConnectionFailure()` (classifies a fetch failure as `'timeout' | 'network' | 'other'`) and a new "Timeout saat terhubung ke server. Coba lagi." message. Unit-tested in `lib/pipeline-messages.test.ts`. This addresses the misleading "Gagal terhubung ke server" that appeared when the server was merely busy — not a real connection failure.
